...
Generic Unit is a natively multichain asset. When users bridge Generic Unit to a Layer 2, the L1 tokens are locked in the bridge contract, and an equivalent amount of Generic Unit is minted on the destination chain. Conversely, when bridging back to L1, the L2 tokens are burned, and the corresponding L1 tokens are released from the bridge.
